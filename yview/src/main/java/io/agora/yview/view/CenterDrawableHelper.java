...
final class CenterDrawableHelper {
    private static void onCenterDrawEdit(TextView view, Canvas canvas, Drawable drawable, int gravity) {
        ...
        switch (gravity) {
            case Gravity.RIGHT:
                ratio = -1;
            case Gravity.LEFT:
                total = view.getPaint().measureText(view.getHint().toString()) + drawable.getIntrinsicWidth() + drawablePadding + view.getPaddingLeft() + view.getPaddingRight();
    ...
    public static void preDrawEdit(TextView view, Canvas canvas) {
        Drawable[] drawables = view.getCompoundDrawables();
        if (drawables != null) {
            if (drawables[0] != null) {
                view.setGravity(Gravity.CENTER_VERTICAL | Gravity.LEFT);
                onCenterDrawEdit(view, canvas, drawables[0], Gravity.LEFT);
            } else if (drawables[1] != null) {
                view.setGravity(Gravity.CENTER_HORIZONTAL | Gravity.TOP);
                onCenterDrawEdit(view, canvas, drawables[1], Gravity.TOP);
            } else if (drawables[2] != null) {
                view.setGravity(Gravity.CENTER_VERTICAL | Gravity.RIGHT);
                onCenterDrawEdit(view, canvas, drawables[2], Gravity.RIGHT);
            } else if (drawables[3] != null) {
                view.setGravity(Gravity.CENTER_HORIZONTAL | Gravity.BOTTOM);
                onCenterDrawEdit(view, canvas, drawables[3], Gravity.BOTTOM);
            }
        }
    }
}

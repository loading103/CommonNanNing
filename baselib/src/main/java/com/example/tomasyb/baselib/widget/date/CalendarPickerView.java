...
public class CalendarPickerView extends ListView {
    ...
    //设置选中项的文字（开始项）
    public String selectStartValue = "入住";
    public String selectEndValue = "离店";

    public void setSelectStartValue(String selectStartValue) {
        this.selectStartValue = selectStartValue;
    }

    public void setSelectEndValue(String selectEndValue) {
        this.selectEndValue = selectEndValue;
    }
    ...
    /**
     * MonthCellDescriptor{date=Tue Dec 19 00:00:00 GMT+08:00 2017, value=19, isCurrentMonth=true,
     * isSelected=false, isToday=false, isSelectable=true, isHighlighted=false, rangeState=NONE}
     */
    private List<CalendarCellView> cellViewList = new ArrayList<>();
    ...
    private class CellClickedListener implements MonthView.Listener {
        ...
        public void handleClick(MonthCellDescriptor cell, View v) {
            ...
            monthList.add(clickedDate.getMonth());
            ...
            if (cell.isToday()) {
                ...
                dataList.add(cellView.getDayOfMonthTextView().getText().toString().split("\\n")[0]);
            }
        }
    }
    ...
    private class MonthAdapter extends BaseAdapter {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            //MonthCellDescriptor
            //点击之后的选中配置
            if (cellViewList.size() > 0 && monthList.size() > 0) {
                if (months.get(position).getMonth() == monthList.get(0)) {
                    cellViewList.get(0).getDayOfMonthTextView().setText(dataList.get(0) + "\n" + selectStartValue);
                }
                if (cellViewList.size() == 2 && monthList.size() == 2) {
                    if (!dataList.get(0).equals(dataList.get(1))) {
                        if (months.get(position).getMonth() == monthList.get(1)) {
                            cellViewList.get(1).getDayOfMonthTextView().setText(dataList.get(1) + "\n" + selectEndValue);
                        }
                    }
                }
            }
            if(months.get(position).getDate().equals(start)){//入住
            }
            if(months.get(position).getDate().equals(end)){//离店
            }
            return monthView;
        }
    }
